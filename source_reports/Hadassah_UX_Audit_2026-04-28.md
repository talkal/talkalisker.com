...
נתיב החיכוך מכניסה לאתר ועד לכפתור קביעת התור הראשון כולל 8 שכבות: 1. מידע על הצוות ← 2. פתולוגיות ← 3. ניתוחים ← 4. שאלות נפוצות ← 5. עוד פתולוגיות ← 6. מרכז ידע למטופל ← 7. מרכז ידע לשיקום ← 8. CTA קבור בתחתית. מחקרים מצביעים על ~70% נטישה בזרימות הדורשות יותר מ-3 צעדים. היחידה מפעילה משפך של 8 צעדים בעולם של 3.
...
- **מידוד:** Mayo Clinic | **תובנה:** ניווט מסימפטום לאבחון ב-3 צעדים עם רשתות ביטחון מובנות
הניווט הדיגיטלי של Mayo מתחיל במיקום האנטומי, ממשיך לאופי הסימפטום, ומשלב סולמות כאב מאומתים בכל נקודת החלטה. באופן קריטי, הזרימה מכילה גילוי חירום מובנה: אם פרופיל הסימפטום תואם לטראומה חריפה או לסימני סכנה לאחר ניתוח — הזרימה נעצרת ומפנה מיידית לטיפול דחוף. זהו המודל למנגנון ניתוב החירום הקליני של הדסה.
...
"מרכז מצוינות — המרכז המנוסה והמתקדם ביותר לכתף בישראל." זה הישג שהושג דרך נפח ניתוחי, מורכבות פרוצדורות ותפוקת מחקר. האתר צריך לשקף זאת מבלי להכריז על כך. ראיות על-פני טענות: תמונות צוות, מספרי פרוצדורות ואזכורי טכנולוגיה ספציפיים מעבירים סמכות בצורה אמינה יותר משפה עמוסת שמות תואר.
...
דוגמאות מיקרו-קופי: "לחץ כאן לזימון תור" ← "הגיע הזמן להחזיר את התנועה לכתף שלך" / "מידע על ניתוחים" ← "למה לצפות ביום הניתוח?" / "הנחיות שיקום" ← "ציר הזמן של ההחלמה שלך — שבוע אחר שבוע."

- **החלטה:** פרוטוקול ז'רגון סמנטי | **השפעה:** בינונית
הכלל: שפת המטופל בכותרות וב-CTAs, טרמינולוגיה קלינית בטקסט המשני וב-Tooltips. "הכתף שלי נוקשה, תפוסה ותקועה" היא הכותרת. "קפסוליטיס דביקה (כתף קפואה)" היא ההפניה הקלינית בסוגריים מתחתיה. מנועי חיפוש מאנדקסים את שניהם. מטופלים מזדהים עם הראשון. רופאים מתאשרים עם השני.
...
- **החלטה:** ניהול חירום ויציאה מהירה — ניתוב חירום קליני | **השפעה:** קריטית
...
- **פעולה:** הטמעת CTA קבוע ומנגנון ניתוב חירום | **אחראי:** צוות פיתוח | **סטטוס:** ממתין
"קביעת תור" חייב להופיע בניווט קבוע בכל מיקום גלילה. שכבת גילוי חירום חייבת לפעול לפני המשך כל ניתוב אוטומטי כאשר מזוהים מחווני סימפטום חריף.

- **פעולה:** עיצוב מחדש של אזור ה-Hero | **אחראי:** ארכיטקט UX | **סטטוס:** ממתין
שורת חיפוש, סלוגן ממוקד מטופל ("הגיע הזמן להחזיר את התנועה לכתף שלך"), ו-CTA ראשי מעל הקפל. הסרת ביוגרפיה מוסדית מהנדל"ן מעל הקפל.
...
צילומי צוות וביוגרפיה מועברים מתחת לתוכן הפתרון הקליני. הערך הקליני מוביל; הפן האנושי תומך ומחזק.
...
- כתיבה מחדש של תוכן הפונה למטופל — מעבר לטון אמפתי לאורך כל הטקסט הקיים
- CTA קבוע — "תאמו פגישת ייעוץ" קבוע בכל מיקום גלילה
...
- שכבת ניתוב חירום — זיהוי סימפטומים חריפים וניתוב חירום
...
```pagezone
Zone: Hero | Type: Hero
כותרת בשפת המטופל: "סובלים מכאב בכתף? זאת המומחיות שלנו." שם יחידת הכתף של הדסה + שיוך לבית החולים כעוגן אמון משני. תיאור יחידה במשפט אחד. CTA ראשי יחיד: "תאמו פגישת ייעוץ" — גלוי מעל הקפל בכל המכשירים.

Zone: בורר כוונות | Type: Navigation
שלושה כפתורים גדולים שווי משקל המתאימים לשלוש כוונות המטופל מנספח ב: "אני סובל/ת מכאב, אי-נוחות או פציעה בכתף." / "אני לקראת ניתוח כתף." / "אני בתהליך החלמה מניתוח כתף." כל אחד מנתב לזרימה הרלוונטית. אין ניווט אחר מתחרה בשלושה נתיבים אלה.

Zone: למה יחידת כתף הדסה | Type: Social Proof
3–4 מדדי אמון: נפח פרוצדורות שנתי, שנות פעילות היחידה, טכנולוגיה בשימוש (ניתוח בסיוע רובוט, הדמיה תלת-ממדית), שיעור מטופלים בינלאומיים. מותג הדסה כגב. סריקה בלבד — ללא טקסט צפוף. תת-טקסט: "אתה בידיים של מומחים."

Zone: נווט פתולוגיות | Type: Navigation
רשת של 6–8 מצבי כתף נפוצים עם תוויות כרטיס בשפת המטופל ("הכתף שלי נוקשה, תפוסה ותקועה" ← כתף קפואה). כל כרטיס מקשר לעמוד הפתולוגיה המתאים. תווית דו-שפתית: כותרת סימפטום בגדול + מונח קליני קטן. נקודת כניסה לרוב המבקרים שמאבחנים עצמם.
...
|---|---|---|
| קפסוליטיס דביקה (כתף קפואה) | "הכתף שלי נוקשה, תפוסה ותקועה." | קושי עמוק בפעילויות יומיומיות עקב כיווץ קפסולרי. |
...
| פריקת מפרק AC | "יש לי בליטה בולטת בחלק העליון של הכתף לאחר נפילה." | כאב ממוקד לחלק העליון של הכתף לאחר טראומה חריפה. |
| קרע / דלקת בגיד הביספס המקורב | "כאב בקדמת הזרוע, או בליטה פתאומית." | כאב קדמי בכתף בעת אחיזה או כפיפת שריר כנגד התנגדות. |
...
| 2 | אני לקראת ניתוח כתף בקרוב. | ← זרימת טרום-ניתוח (שלב 2ב) |
| 3 | אני בתהליך החלמה מניתוח כתף. | ← זרימת לאחר-ניתוח (שלב 2ג) |
...
| 4 | צפייה בסרטוני הדרכה להכנת סביבת המגורים. | הפניה לווידאו הדרכה לקראת ניתוח החלפת כתף / ארתרוסקופיה. |
...
|---|---|---|---|
| 1 | יש לי בליטה בולטת בחלק העליון של הכתף. | פריקת מפרק AC / שבר בעצם הבריח. | הפניה לטיפול אורתופדי דחוף. |
| 2 | כאב חמור ואיני יכול/ה להרים את היד. | קרע מאסיבי חד / פריקה. | הפניה להערכה חריפה ודימות. |
| 3 | בליטה בשריר הזרוע וכאב בקדמת הכתף. | קרע גיד הביספס המקורב. | הפניה למידע וזימון תור למומחה. |
...
| # | תיאור הסימפטום | אבחון קליני | פעולה מומלצת |
|---|---|---|---|
| 1 | מעיר בלילה, במיוחד כשישנים על הצד. | דלקת / קרע בשרוול המסובב. | הפניה למידע והדרכה על השרוול המסובב וזימון לאולטרסאונד/MRI. |
...
|---|---|---|---|
| כתף קפואה (Adhesive Capsulitis) | "נוקשה, תפוסה ותקועה. חוסר תנועה עמוק עקב כיווץ קפסולרי." | פיזיותרפיה אינטנסיבית והזרקות למפרק. | שחרור ארתרוסקופי של הקפסולה למקרים עמידים. |
...
| יום הניתוח | הסרת תכשיטים ופירסינג, בגדים נוחים ורפויים, זמני הגעה מדויקים. | הבטחת בטיחות פיזית ונוחות לאחר הניתוח. |
...
| מנגנון ניתוב חירום | התראת חירום בולטת | זיהוי סימפטומים חריפים והפניה מיידית למיון. |
...
| מיקרו-קופי | תוויות מוכוונות-פעולה ואמפתיות | "הגיע הזמן להחזיר את התנועה לכתף שלך". |
